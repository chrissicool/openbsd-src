...
#define	FRAME_SIZE	(IBT_SIZE + RTGD_MOV_SIZE + RTGD_XOR_SIZE + 1 + 3)
...
db_epilogue_validate(Elf_Sym *symp)
{
	...
	while (off > FRAME_SIZE + 2) {
		if (inst[off - 1] == BKPT_INST && inst[off] == RET_INST)
			return off;
		off--;
	}
...
db_epilogue_validate(Elf_Sym *symp)
{
	 ...
	 */
	while(*((uint8_t *)inst) == BKPT_INST)
		(*(uint8_t *)inst) -= 1;
#endif
	if (*(uint8_t *)(limit - 2) != POP_RBP_INST)
		return -1;
...
db_epilogue_patch(vaddr_t addr, int restore)
{
	...
	CTASSERT(POP_RBP_SIZE == BKPT_SIZE);
	if (restore) {
		patch = POP_RBP_INST;
		size = POP_RBP_SIZE;
	} else {
